...
export let OpenLock = async function () {
    ...
    for (let targetToken of game.user.targets) {
        ...
        if (targetToken.actor.getFlag('lootsheetnpc5e', 'lootsheettype') === 'Loot') {
            ...
            //Tem uma trance ent�o verificar se possui a chave ou se precisa de uma.
            // Tem um a tranca, n�o possuo uma chave, ent�o verificar se h� uma armadilha.
            let hasKey = HasKey(hasLock, actor);
            if (hasKey) {
                // Se precisa de uma chave e eu tenho a chave n�o preciso checar mais nada.
                let options = { open: true, trap: false, disarm: false, remove: false }
                let msg = {
                    SearchTitile: game.i18n.localize('OpenLock.MsgChat.OpenTitle'),
            ...
            let hasOpen = false; let trapDisarm = false; let trapRemove = false; let toolsbreak = false;// Se tem tranca ent�o esta fechado para quem n�o tem permiss�o ou a chave
            // Verificar se h� alguma armadilha configurada
            let haveTrap = (hasLock.data.data.actionType !== '') ? true : false;
            // Daqui em diante Sup�es-se esta fechado trancado e pode haver ou n�o uma armadilha
...
let CheckPermission = async (targetToken) => {
    ...
    //console.log("N�o tenho permiss�o");
    return false;
}
